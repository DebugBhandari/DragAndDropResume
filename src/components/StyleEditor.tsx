...
export default function StyleEditor() {
  const { style, setStyle, photo, setPhoto, activeLocale } = useResumeStore();
  ...
  return (
    <div className="space-y-3">
      <div className="rounded-lg border border-gray-200 bg-gray-50/60 p-3 space-y-3">
        <p className="text-[11px] font-semibold uppercase tracking-wider text-gray-500">Theme</p>
        <div>
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-3 space-y-3">
        <p className="text-[11px] font-semibold uppercase tracking-wider text-gray-500">Typography</p>
        <div>
          ...
          <div className="flex gap-2">
            {(['serif', 'sans-serif', 'mono'] as const).map((f) => (
              <button
                key={f}
                onClick={() => setStyle({ fontFamily: f })}
                className={`style-chip ${style.fontFamily === f ? 'style-chip-active' : ''}`}
      ...
      <div className="rounded-lg border border-gray-200 bg-blue-50/50 p-3 space-y-3">
        <p className="text-[11px] font-semibold uppercase tracking-wider text-gray-500">Header Layout</p>
        <div>
        ...
        <div>
          <label className="text-xs font-medium text-gray-600 block mb-1">
            Header height: {photo.headerHeight ?? 180}px
          </label>
          <input
            type="range"
            min={80}
            max={360}
            value={photo.headerHeight ?? 180}
            onChange={(e) => setPhoto({ headerHeight: +e.target.value })}
            className="w-full"
          />
        </div>
        ...
        <div>
          <label className="text-xs font-medium text-gray-600 block mb-1">
            Header top margin: {photo.marginTop ?? 0}px
          </label>
          <input
            type="range"
            min={0}
            max={48}
            value={photo.marginTop ?? 0}
            onChange={(e) => setPhoto({ marginTop: +e.target.value })}
            className="w-full"
          />
        </div>
        ...
        <div>
          <label className="text-xs font-medium text-gray-600 block mb-1">
            Header bottom margin: {photo.marginBottom ?? 0}px
          </label>
          <input
            type="range"
            min={0}
            max={48}
            value={photo.marginBottom ?? 0}
            onChange={(e) => setPhoto({ marginBottom: +e.target.value })}
            className="w-full"
          />
        </div>
      </div>
